...
// Pakai ID unik khusus pintu (bukan 1 lagi)
const DOOR_BACK_ID = 901;
...
export default function RoomOrtuScene({
  onBackHallway,
  mapImageSrc = "/assets/maps/kamarortu.png",
  playerImageSrc = "/assets/characters/backleft.png",
}) {
  const [obstacles, setObstacles] = useState(() =>
    loadRoomOrtuObstaclesFromStorage() ?? getDefaultRoomOrtuObstacles()
  );
  useEffect(() => {
    localStorage.setItem(RO_STORAGE_KEY, JSON.stringify(obstacles));
  }, [obstacles]);
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      // Cek dekat pintu (atas/bawah/kanan/kiri sama-sama terbaca)
      const playerBox = {
        x: moved.x,
        y: moved.y,
        width: moved.w,
  ...
  return (
    <div className="hw-root">
      <div className="hw-stage" ref={worldRef}>
        <img src={mapImageSrc} alt="room-ortu" className="hw-map pixelated" />
        <img
          ref={spriteRef}
          src={playerImageSrc}
          alt="player"
