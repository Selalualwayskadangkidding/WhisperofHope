...
import {
  RK_STORAGE_KEY,
  getDefaultRoomKakakObstacles,
  loadRoomKakakObstaclesFromStorage,
} from "../data/roomKakakObstacles.js";
import "../styles/hallway.css";

const HITBOX = { offsetX: 65, offsetY: 135, w: 20, h: 15 };
const SPEED = 180;

// Pintu balik → hallway (ID unik & type "door")
const DOOR_BACK_ID = 900;

// AABB overlap yang konsisten (x,y,width,height)
...
export default function RoomKakakScene({
  onBackHallway,
  mapImageSrc = "/assets/maps/kamarkk.png",
  playerImageSrc = "/assets/characters/backleft.png",
}) {
  const [obstacles, setObstacles] = useState(() =>
    loadRoomKakakObstaclesFromStorage() ?? getDefaultRoomKakakObstacles()
  );
  useEffect(() => {
    localStorage.setItem(RK_STORAGE_KEY, JSON.stringify(obstacles));
  }, [obstacles]);
  ...
  // Ambil rect pintu (type: "door", id: 900)
  const doorRect = useMemo(() => {
    const d = obstacles.find(
      (o) => o.type === "door" && o.id === DOOR_BACK_ID
    );
    return d
  ...
  useDoorEnter({
    enabled: near,
    onEnter: () => onBackHallway && onBackHallway({spawnAtDoorId:7}),
  });
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      // cek overlap dengan pintu (simetris, atas/bawah OK)
      const playerBox = {
        x: moved.x,
        y: moved.y,
        width: moved.w,
  ...
  return (
    <div className="hw-root">
      <div className="hw-stage" ref={worldRef}>
        <img src={mapImageSrc} alt="room-kakak" className="hw-map pixelated" />
        <img
          ref={spriteRef}
          src={playerImageSrc}
          alt="player"
