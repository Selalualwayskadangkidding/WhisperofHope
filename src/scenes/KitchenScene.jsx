...
import {
  KT_STORAGE_KEY,
  getDefaultKitchenObstacles,
  loadKitchenObstaclesFromStorage,
} from "../data/kitchenObstacles.js";
import "../styles/hallway.css";

const HITBOX = { offsetX: 65, offsetY: 135, w: 20, h: 15 };
const SPEED = 180;

// ID unik khusus pintu balik → hallway
const DOOR_BACK_ID = 903;
...
export default function KitchenScene({
  onBackHallway,
  mapImageSrc = "/assets/maps/dapur.jpg",
  playerImageSrc = "/assets/characters/backleft.png",
}) {
  const [obstacles, setObstacles] = useState(() =>
    loadKitchenObstaclesFromStorage() ?? getDefaultKitchenObstacles()
  );
  useEffect(() => {
    localStorage.setItem(KT_STORAGE_KEY, JSON.stringify(obstacles));
  }, [obstacles]);
  ...
  return (
    <div className="hw-root">
      <div className="hw-stage" ref={worldRef}>
        <img src={mapImageSrc} alt="kitchen" className="hw-map pixelated" />
        <img
          ref={spriteRef}
          src={playerImageSrc}
          alt="player"
